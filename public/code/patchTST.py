...
class self_att(nn.Module):
    def __init__(self, dim_input, dropout):
        super(self_att, self).__init__()
        self.query = nn.Conv1d(in_channels= 2 * dim_input, out_channels=2 * dim_input, kernel_size=1)
        self.key = nn.Conv1d(in_channels=2 * dim_input, out_channels=2 * dim_input, kernel_size=1)
        self.value = nn.Conv1d(in_channels=2 * dim_input, out_channels=2 * dim_input, kernel_size=1)
        self.laynorm = nn.LayerNorm([2 * dim_input])
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, x):
        ...
        v = self.dropout(self.value(x))
        kd = torch.sqrt(torch.tensor(k.shape[-2]).to(torch.float32))
        line = self.dropout(self.softmax(q @ k / kd)) @ v
